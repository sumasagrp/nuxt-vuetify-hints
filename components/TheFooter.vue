...
<template>
  <div
    v-show="!hideFooter" class="mb-12 mt-n1 >
    <VFooter>
  ...
  >
    <!-- ** Skriptjobs logo  -->
    <div
      class="mb-10 md:mb-5 prevent-select md:w-8/12 lg:w-4/12 text-lg mr-7>
            <p class="my-2
    >
  ...
  <div
    :key="name>
              <h2 class="class="grid grid-cols-2 gap-8 sm:grid-cols-3>
            <div v-for=" { name, children } in columns"mb-6 text-sm font-semibold uppercase
  >
    {{ name }}
    </h2>
    ...
    <ul>
      <li
        v-for="{ name: childName, link } in children" :key="childName" class="mb-4>
      ...
      >
        ...
        >
          <a
            class="hover:underline" href="#> Terms of Use </a>
              ...
              <!-- ** Made with ...  -->
              <div>
                Made with <span style="font-size: 13px
          > 💜</span> in Sweden
            <span
